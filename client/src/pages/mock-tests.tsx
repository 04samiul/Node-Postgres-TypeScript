...
export default function MockTestsPage() {
  useSEO({
    title: "Mock Tests",
    description:
      "Practice with realistic Chittagong University admission mock tests. Timed exams with detailed grading for English, Analytical Skills, and Problem Solving.",
    path: "/mock-tests",
  });
  ...
  const filtered =
    mockTests?.filter((t) => {
      ...
      if (filter === "Free")
        return (t.access === "all" || t.access === "signin") && !t.courseId;
      return t.tag === filter;
    }) ?? [];
    ...
    <div className="max-w-7xl mx-auto px-4 py-8" data-testid="page-mock-tests">
      ...
      >
        <h1
          ...
          data-testid="heading-mock-tests"
        >
          Mock Tests
        </h1>
        <p className="text-muted-foreground mb-6">
          Practice with our mock tests for CU admission
        </p>
      </motion.div>
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <CardTitle
                        ...
                        data-testid={`text-mocktest-title-${test.id}`}
                      >
                        {test.title}
                      </CardTitle>
                  ...
                  <CardContent className="flex-1">
                    ...
                    {draft && !isUpcoming && (
                      <div className="mt-2 flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400 font-medium">
                        ...
                        <span>
                          {Object.keys((draft.answers as object) || {}).length}{" "}
                          answered · Unsubmitted
                        </span>
                      </div>
                    )}
                    {subs.length > 0 &&
                        ...
                        return (
                          <div className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                            ...
                            <span>
                              ...
                              · Latest: {latest?.netMarks?.toFixed(1)}
                            </span>
                            <Badge
                              variant={
                                latest?.passed ? "default" : "destructive"
                              }
      ...
      <Dialog
        open={previewMockId !== null}
        onOpenChange={(open) => {
          if (!open) setPreviewMockId(null);
        }}
      >
        <DialogContent
          className="max-w-sm"
          data-testid="dialog-preview-attempts"
        >
          <DialogHeader>
          ...
          <div className="space-y-2 mt-1">
            {previewSubmissions.map((sub, idx) => {
              const attemptLabel =
                ORDINALS[previewSubmissions.length - 1 - idx] ??
                `${previewSubmissions.length - idx}th`;
              return (
                <div
                  key={sub.id}
                  className="flex items-center justify-between gap-2 p-2 rounded-md bg-muted/50"
                  data-testid={`preview-attempt-row-${sub.id}`}
                >
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      ...
                      className={sub.passed ? "bg-green-600" : ""}
                    >
                      {sub.netMarks?.toFixed(1)}
                    </Badge>
                    <Link
                      href={`/mock-review/${sub.id}`}
                      onClick={() => setPreviewMockId(null)}
                    >
                      <Button
                        size="sm"
                        variant="outline"
                        data-testid={`button-review-attempt-${sub.id}`}
                      >
    ...
    <>
      ...
      <Link href={`/mock-tests/${test.id}/leaderboard`}>
        <Button
          size="sm"
          variant="outline"
          data-testid={`button-leaderboard-${test.id}`}
        >
        ...
        <>
          <Link href={`/courses/${test.courseId}`}>
            <Button
              size="sm"
              variant="outline"
              data-testid={`button-enroll-access-${test.id}`}
            >
              <Lock className="h-3.5 w-3.5 mr-1" />
              Enroll to Access
            </Button>
    ...
    return (
      <>
        <Button
          ...
          variant="outline"
          disabled
          data-testid={`button-premium-${test.id}`}
        >
          Premium Only
        </Button>
        {previewBtn}
